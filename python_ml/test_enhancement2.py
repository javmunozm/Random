#!/usr/bin/env python3
"""
Test Enhancement 2: Proportional Scoring Bonuses

Tests if proportional multipliers (vs flat bonuses) improve performance.
"""

import json
from pathlib import Path
from true_learning_model_enhanced2 import TrueLearningModelEnhanced2
...
def run_test():
    """Run Enhancement 2 test"""

    print("=" * 80)
    print("ENHANCEMENT 2 TEST: Proportional Scoring Bonuses")
    print("=" * 80)
    print()
    print("Changes from Phase 1 Pure:")
    print("  - Sum optimal (180-189): Flat +0.15 → 1.05x multiplier (~5% boost)")
    print("  - Even/odd balance (6-8): Flat +2.0 → 1.03x multiplier (~3% boost)")
    print("  - Gap preference (45%+ gap-1): NEW 1.02x multiplier (~2% boost)")
    print()
    print("Rationale: Flat bonuses were 0.3-0.7% of score (invisible)")
    print("           Proportional multipliers scale with score (effective)")
    print()
    print("Expected: Bonuses that actually matter in final scoring")
    ...
    # Initialize enhanced model v2
    model = TrueLearningModelEnhanced2()
    ...
    # Comparison
    baseline_best = 0.714
    enhancement1_best = 0.679
    improvement_vs_baseline = avg_best - baseline_best
    improvement_vs_enh1 = avg_best - enhancement1_best

    print("=" * 80)
    print("COMPARISON")
    print("=" * 80)
    print()
    print(f"Phase 1 Pure Baseline:  {baseline_best:.1%}")
    print(f"Enhancement 1:          {enhancement1_best:.1%} ({enhancement1_best - baseline_best:+.1%})")
    print(f"Enhancement 2 Result:   {avg_best:.1%}")
    print(f"vs Baseline:            {improvement_vs_baseline:+.1%}")
    print(f"vs Enhancement 1:       {improvement_vs_enh1:+.1%}")
    print()

    if improvement_vs_baseline > 0.01:  # >1% improvement
        print("✅ SUCCESS: Enhancement 2 beats baseline!")
        print(f"   Proportional scoring works!")
    elif improvement_vs_baseline > 0:
        print("⚠️  MARGINAL: Small improvement over baseline")
    else:
        print("❌ NO IMPROVEMENT: Enhancement 2 doesn't beat baseline")

    if improvement_vs_enh1 > 0:
        print(f"   ✅ Better than Enhancement 1 by {improvement_vs_enh1:+.1%}")
    else:
        print(f"   ❌ Worse than Enhancement 1 by {improvement_vs_enh1:+.1%}")
