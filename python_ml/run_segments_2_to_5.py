#!/usr/bin/env python3
"""
Run segments 2, 3, 4, and 5 in parallel for the exhaustive search
"""
...
def run_segment_worker(args):
    """
    Worker function to run one segment of the exhaustive search

    Args:
        args: tuple of (model_weights, segment_num, start_idx, end_idx, series_id)

    Returns:
        dict with best combination and score for this segment
    """
    model_weights, segment_num, start_idx, end_idx, series_id = args
    ...
    # Recreate model with loaded weights
    model = TrueLearningModel(seed=999, cold_hot_boost=30.0)
    model.RECENT_SERIES_LOOKBACK = 8

    # Restore model weights
    model.number_frequency_weights = model_weights['number_frequency_weights']
    model.position_weights = model_weights['position_weights']
    model.pattern_weights = model_weights['pattern_weights']
    model.pair_affinities = model_weights['pair_affinities']
    model.number_avoidance = model_weights['number_avoidance']
    model.critical_numbers = model_weights['critical_numbers']
    model.recent_series_data = model_weights['recent_series_data']
    ...
    for combination in islice(all_combos, segment_size):
        ...
        if score > best_score:
            ...
            print(f"  🎯 New best in Seg {segment_num}: score={score:.2f}, combo={combo_list[:3]}...{combo_list[-3:]}, time={elapsed:.1f}s")
        ...
        if count >= next_report:
            ...
            print(f"  Seg {segment_num} Progress: {count:,}/{segment_size:,} ({progress_pct:.1f}%) - "
    ...
    intermediate_file = Path(__file__).parent / f'exhaustive_segment_{segment_num}_results.json'
    with open(intermediate_file, 'w') as f:
        json.dump({
            'segment': result,
        }, f, indent=2)

    print(f"💾 Segment {segment_num} results saved to: {intermediate_file}")

    return result


def run_segments_2_to_5(series_id: int = 3147):
    """
    Run segments 2-5 in parallel for the exhaustive search
    """
    print("=" * 80)
    print("PARALLEL EXHAUSTIVE SEARCH - SEGMENTS 2-5")
    print("=" * 80)
    print()
    print(f"Target Series: {series_id}")
    print(f"Running segments: 2, 3, 4, 5")
    print(f"Total Combinations: ~3,565,920 (80% of 4,457,400)")
    print()
    ...
    # Extract model weights for sharing with workers
    model_weights = {
        'number_frequency_weights': dict(model.number_frequency_weights),
        'position_weights': dict(model.position_weights),
        'pattern_weights': dict(model.pattern_weights),
        'pair_affinities': dict(model.pair_affinities),
        'number_avoidance': {k: dict(v) for k, v in model.number_avoidance.items()},
        'critical_numbers': dict(model.critical_numbers),
        'recent_series_data': list(model.recent_series_data)
    }
    ...
    worker_args = [
        (model_weights, seg_num, start_idx, end_idx, series_id)
    ...
    # Use 4 processes (one per segment)
    with mp.Pool(processes=4) as pool:
        all_results = pool.map(run_segment_worker, worker_args)

    end_time = time.time()
    total_time = end_time - start_time

    # Load segment 1 results
    seg1_file = Path(__file__).parent / 'exhaustive_segment_1_results.json'
    with open(seg1_file, 'r') as f:
        seg1_data = json.load(f)
        seg1_result = seg1_data['segment']

    # Combine with segment 1
    all_results = [seg1_result] + all_results
    ...
    for result in all_results:
        if result['best_score'] > overall_best_score:
            ...
            overall_best_segment = result['segment']
    ...
    total_time_all = sum(r['time_seconds'] for r in all_results)
    total_scored = sum(r['combinations_scored'] for r in all_results)
    avg_rate = total_scored / total_time_all

    print(f"Total combinations scored: {total_scored:,}")
    print(f"Total time (parallel): {total_time:.1f} seconds ({total_time/60:.2f} minutes)")
    print(f"Total time (sum of all segments): {total_time_all:.1f} seconds ({total_time_all/60:.2f} minutes)")
    ...
    final_results = {
        'series_id': series_id,
        'method': 'exhaustive_mandel_segmented_parallel',
...
if __name__ == '__main__':
    results = run_segments_2_to_5(series_id=3147)
