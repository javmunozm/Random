...
Statistical analysis of Python model performance vs C# baseline
"""
...
def run_single_simulation(seed, all_series_data, test_series, training_end):
    ...
    Args:
        ...
        all_series_data: Full dataset
        test_series: List of series IDs to test on
        training_end: Train on all series before this ID
...
def run_comprehensive_simulation(num_simulations=10000, test_start=3141):
    ...
    Args:
        ...
        test_start: First series ID to test (will test from this to 3150)
    """
    print("=" * 80)
    print("COMPREHENSIVE SIMULATION - 10,000+ RUNS")
    print("=" * 80)
    print()
    ...
    if test_start > max_series:
        test_start = max_series - 9  # Ensure at least 10 test series

    test_series = [sid for sid in range(test_start, max_series + 1)]
    training_end = test_start  # Train on all before test_start
    ...
    print(f"Progress: 0/{num_simulations}", end="", flush=True)
    ...
    for i in range(num_simulations):
        ...
        if (i + 1) % 100 == 0:
            print(f"\rProgress: {i+1}/{num_simulations}", end="", flush=True)

    print(f"\rProgress: {num_simulations}/{num_simulations} ✅")
    print()
...
def compare_to_csharp(stats, test_series):
    ...
    # Statistical significance test (vs C# average)
    # Using z-test since we have large sample size
...
def main():
    ...
    # Run 10,000 simulations from Series 3141 onwards
