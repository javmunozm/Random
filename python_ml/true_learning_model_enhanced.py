#!/usr/bin/env python3
"""
TrueLearningModel - ENHANCED (Phase 1+)

Enhancement 1: Boosted Affinity Scoring
- Increased pair affinity: 25.0x → 30.0x
- Increased triplet affinity: 35.0x → 45.0x
- Added quad affinity tracking: 55.0x (4-number patterns)

Based on data showing 33.5% pair co-occurrence - patterns are real!
"""
...
class TrueLearningModelEnhanced:
    """Phase 1+ with enhanced affinity scoring"""
    ...
    # ENHANCEMENT 1: Boosted affinity multipliers
    PAIR_AFFINITY_MULTIPLIER = 30.0  # Was 25.0
    TRIPLET_AFFINITY_MULTIPLIER = 45.0  # Was 35.0
    QUAD_AFFINITY_MULTIPLIER = 55.0  # NEW: 4-number patterns
    ...
    def _update_weights(self, series_id: int, combinations: List[List[int]]):
        ...
        # Learn pair, triplet, and quad affinities from each event
        for combo in combinations:
            self._learn_pair_affinities(combo)
            self._learn_triplet_affinities(combo)
            self._learn_quad_affinities(combo)  # NEW
    ...
    def validate_and_learn(self, series_id: int, prediction: List[int], actual_results: List[List[int]]):
        ...
        for actual_event in actual_results:
            ...
            self._learn_quad_affinities(list(actual_event))  # NEW
        ...
        if self.quad_affinities:  # NEW
            top_quads = sorted(self.quad_affinities.items(), key=lambda x: x[1], reverse=True)[:2]
            print(f"🔗🔗🔗🔗 Top 2 quad affinities: {', '.join(f'{a:02d}+{b:02d}+{c:02d}+{d:02d}' for (a, b, c, d), _ in top_quads)}")
    ...
    def _learn_quad_affinities(self, combination: List[int]):
        """NEW: Learn 4-number patterns"""
        for i in range(len(combination)):
            for j in range(i + 1, len(combination)):
                for k in range(j + 1, len(combination)):
                    for m in range(k + 1, len(combination)):
                        quad = tuple(sorted([combination[i], combination[j], combination[k], combination[m]]))
                        self.quad_affinities[quad] = self.quad_affinities.get(quad, 0.0) + self.learning_rate
    ...
    def _calculate_quad_affinity_score(self, combination: List[int]) -> float:
        """NEW: Calculate quad affinity score"""
        if not self.quad_affinities:
            return 0.0
        ...
        for i in range(len(combination)):
            for j in range(i + 1, len(combination)):
                for k in range(j + 1, len(combination)):
                    for m in range(k + 1, len(combination)):
                        quad = tuple(sorted([combination[i], combination[j], combination[k], combination[m]]))
                        if quad in self.quad_affinities:
                            affinity_score += self.quad_affinities[quad]

        return affinity_score * self.QUAD_AFFINITY_MULTIPLIER
