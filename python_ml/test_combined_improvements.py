#!/usr/bin/env python3
"""
Test combined improvements based on what we've learned.

Best improvement so far: 50 recent series only (+1.4%)

Now testing combinations:
1. 50 recent + different learning rates (0.05, 0.15, 0.20)
2. 50 recent + disable pair affinity
3. 50 recent + larger candidate pool
"""
...
def test_config(config_name, modify_model_fn, recent_series_count=50, seed=999):
...
def main():
    ...
    print("="*80)
    print()
    print("Baseline: 50 recent series, 57.1% actual average")
    print("Testing combinations with other tweaks")
    print("="*80)

    baseline_actual = 0.571  # 50 recent series baseline
    all_results = []

    # Test 1: Baseline (50 recent, default settings)
    result = test_config(
        "50 recent (baseline)",
        None,
    ...
    # Test 2: 50 recent + lower learning rate
    def set_lr_low(model):
    ...
    # Test 3: 50 recent + higher learning rate
    def set_lr_high(model):
    ...
    # Test 4: 50 recent + disable pair affinity
    def disable_pair_affinity(model):
        model.pair_affinity_weight = 0.0
    result = test_config(
        "50 recent + No Pair Affinity",
        disable_pair_affinity,
        recent_series_count=50
    )
    all_results.append(result)

    # Test 5: 50 recent + larger candidate pool
    def larger_pool(model):
        model.CANDIDATE_POOL_SIZE = 50000
        model.CANDIDATES_TO_SCORE = 5000
    result = test_config(
        "50 recent + 5k candidates",
        larger_pool,
        recent_series_count=50
    )
    all_results.append(result)

    # Test 6: 50 recent + multiple tweaks combined
    def combined_tweaks(model):
        model.learning_rate = 0.15
        model.CANDIDATE_POOL_SIZE = 50000
        model.CANDIDATES_TO_SCORE = 5000
    result = test_config(
        "50 recent + LR=0.15 + 5k candidates",
        combined_tweaks,
        recent_series_count=50
    )
    all_results.append(result)
    ...
    print("="*80)
    print()
    print("Configuration                              | ACTUAL Avg | vs Baseline")
    print("-------------------------------------------|------------|------------")
    ...
    for result in sorted_results:
        ...
        print(f"  {config:40} |     {actual:5.1f}% | {verdict}")
    ...
    if best['actual_avg'] > baseline_actual + 0.01:
        print(f"✅ IMPROVEMENT FOUND: {best['config_name']}")
        ...
        print(f"   RECOMMENDATION: Implement this configuration")
    else:
        print(f"➖ NO FURTHER IMPROVEMENT")
        print(f"   Best: {best['config_name']}, {best['actual_avg']*100:.1f}%")
        print(f"   RECOMMENDATION: Stick with 50 recent series baseline")

    # Save results
    output_file = "test_combined_improvements_results.json"
