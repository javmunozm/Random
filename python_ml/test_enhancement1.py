#!/usr/bin/env python3
"""
Test Enhancement 1: Boosted Affinity Scoring

Tests if enhanced affinity multipliers improve performance over baseline.
"""

import json
from pathlib import Path
from true_learning_model_enhanced import TrueLearningModelEnhanced
...
def run_test():
    """Run Enhancement 1 test"""

    print("=" * 80)
    print("ENHANCEMENT 1 TEST: Boosted Affinity Scoring")
    print("=" * 80)
    print()
    print("Changes from Phase 1 Pure:")
    print("  - Pair affinity: 25.0x → 30.0x (+20%)")
    print("  - Triplet affinity: 35.0x → 45.0x (+28.6%)")
    print("  - NEW: Quad affinity: 55.0x (4-number patterns)")
    print()
    print("Expected: Better capture of recurring number patterns")
    ...
    model = TrueLearningModelEnhanced()
    ...
    print(f"Enhancement 1 Result:   {avg_best:.1%}")
    print(f"Improvement:            {improvement:+.1%}")
    print()

    if improvement > 0.01:  # >1% improvement
        print("✅ SUCCESS: Enhancement 1 beats baseline!")
        print(f"   Affinity boosting works - patterns matter!")
    elif improvement > 0:
        print("⚠️  MARGINAL: Small improvement, may be noise")
    else:
        print("❌ NO IMPROVEMENT: Enhancement 1 doesn't help")
        print("   Baseline affinity multipliers were already optimal")
