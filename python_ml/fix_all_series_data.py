#!/usr/bin/env python3
"""
Fix all incorrect series data in full_series_data.json
Correct series: 3009, 3072, 3073, 3074, 3103, 3106
"""
...
print("Fixing all incorrect series data...")
print(f"Total series before: {len(data)}")

# Complete correct data provided by user
corrections = {
    "3009": [
    ...
    "3073": [
        [3, 4, 5, 6, 9, 10, 14, 15, 16, 18, 19, 20, 21, 24],
        [2, 3, 4, 5, 6, 10, 11, 12, 13, 14, 16, 20, 21, 22],
        [1, 2, 3, 5, 6, 9, 11, 12, 13, 18, 19, 21, 22, 24],
        [1, 3, 4, 5, 8, 10, 11, 12, 14, 17, 18, 21, 23, 24],
        [2, 4, 5, 7, 9, 10, 11, 15, 16, 20, 22, 23, 24, 25],
        [2, 6, 7, 8, 9, 10, 12, 16, 17, 18, 19, 21, 23, 25],
        [2, 3, 4, 6, 8, 11, 12, 13, 14, 18, 22, 23, 24, 25]
    ],
    "3074": [
        [3, 6, 7, 10, 11, 13, 14, 15, 16, 17, 20, 22, 23, 24],
        [2, 3, 5, 9, 10, 14, 16, 17, 18, 19, 20, 22, 23, 24],
        [3, 4, 7, 10, 11, 12, 16, 17, 19, 20, 21, 22, 23, 24],
        [2, 3, 4, 5, 7, 8, 11, 15, 17, 18, 19, 20, 22, 25],
        [1, 3, 4, 5, 9, 11, 12, 13, 15, 17, 18, 19, 22, 25],
        [2, 3, 6, 9, 10, 11, 14, 16, 17, 20, 21, 23, 24, 25],
        [1, 7, 8, 10, 11, 13, 14, 16, 17, 18, 21, 22, 24, 25]
    ],
    "3103": [
...
for series_id, events in corrections.items():
    if series_id in data:
        ...
        changed = old_data != events
        if changed:
            print(f"\n✓ Updated series {series_id}")
            # Show first event comparison
            print(f"  Old event 1: {sorted(old_data[0])}")
            print(f"  New event 1: {sorted(events[0])}")
            # Show event 2 comparison (where the duplicate was)
            if len(old_data) > 1 and len(events) > 1:
                if old_data[1] != events[1]:
                    print(f"  Old event 2: {sorted(old_data[1])}")
                    print(f"  New event 2: {sorted(events[1])}")
            updated_count += 1
        else:
            print(f"✓ Series {series_id} already correct")
    else:
...
print(f"\n{'='*80}")
print(f"✅ Corrected data saved to full_series_data.json")
print(f"Total series: {len(data)}")
print(f"Series updated: {updated_count}")
print(f"Series corrected: 3009, 3072, 3073, 3074, 3103, 3106")
print(f"{'='*80}")
